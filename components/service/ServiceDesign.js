...
// // Main Component
// export default function ServiceDesign({service}) {
//   return (
...
// services prop expected as { success, data: [...] }
export default function ServiceDesign({ services }) {
  return (
    <section className="max-w-[1800px] w-full mx-auto px-2 md:px-8 py-12 font-sans">
      <div className="flex items-baseline justify-between mb-8">
        {/* <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">
      ...
      <ol className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {services?.data?.map((service) => (
          <ShotCard
            ...
            shot={{
              id: service._id,
              image: service.images?.[0] || "", // First image only
              title: service.title,
              // You can customize the user object or leave as dummy if not available
              user: {
              ...
              likes: service.likes, // You might want to use price or another field
              views: service.views,
              pro: false,
              team: false,
              href: `/services/view/${service.slug}`,
            }}
          />
        ))}
      </ol>
